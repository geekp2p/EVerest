...
git rev-parse --is-inside-work-tree >NUL 2>&1 || (echo Not a Git repository here.& exit /b 1)
...
if !errorlevel! EQU 0 (
  ...
  git commit -m "!msg!" || (echo Commit failed.& exit /b 1)
)
...
:PUSH_NORMAL
set "TMP_PUSH_OUT=%TEMP%\push_out_%RANDOM%.log"
git push >"%TMP_PUSH_OUT%" 2>&1
set "rc=!errorlevel!"
type "%TMP_PUSH_OUT%"
findstr /C:"GH007" "%TMP_PUSH_OUT%" >NUL 2>&1 && (
